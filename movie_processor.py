...
def generate_thumbnails_by_opencv(output_dir, filename, output_mp4_name, thumbnail_interval):
...
def process_single_video(filename, end_time, base_video_path, output_dir, start_time, thumbnail_interval):
    ...
    generate_thumbnails_by_opencv(output_dir, filename, output_file_name, thumbnail_interval)


def process_mp4_files(name_time_list, base_video_path, output_dir, thumbnail_interval, max_workers):
    """ メイン処理を行う関数
    ...
    """
    create_thumbnails = True if thumbnail_interval > 0 else False
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        start_time = 0
        futures = []
        for filename, end_time in name_time_list:
            future = executor.submit(
                process_single_video, filename, end_time, base_video_path, output_dir, start_time, thumbnail_interval
            )
